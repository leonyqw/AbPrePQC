...
class ngi_metadata:
    ...
    def get_ngi_project_metadata(self, pid):
        ...
        if config.filename:
            if config.filename == "seqinspector":
                infix = "seqinspector"
            else:
    ...
    def general_stats_sample_meta(self):
        ...
        if meta is not None and len(meta) > 0:
                ...
                if seq_lp is not None:
                    ...
                    try:
                        for lv in sorted(meta[sid]["library_prep"][lp]["library_validation"].keys()):
                            gsdata[s_name]["lp_concentration"] = meta[sid]["library_prep"][lp]["library_validation"][
                                lv
                            ]["concentration"]
            ...
            report.general_stats_data["NGI_after"] = {
                SampleGroup(sname): [InputRow(sample=SampleName(sname), data=data)] for sname, data in gsdata.items()
            }
